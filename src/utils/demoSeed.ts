...
// Seeds a few example decisions when someone finishes onboarding, so the app
// isn't empty on first run. Speaks the app's existing vocabulary (decisions +
// options) and goes through the normal store actions — no special data format.
...
export function seedDemoDecisions(projectId: ID): void {
  const store = useAppStore.getState();

  for (const demo of DEMO_DECISIONS) {
